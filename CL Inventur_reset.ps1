...
function Update-Progress($i, $Tasks) {
    ...
    $progressText = "Informationssammlung für die Client-Umstellung"
    Write-Progress -Activity $progressText -Status "In Progress" -PercentComplete (($i / $Tasks) * 100)
}
Set-ExecutionPolicy -ExecutionPolicy Bypass
#---------------------------------------------------------[Logic]--------------------------------------------------------
Write-Log "Starte vorbereitung für die Client umstellung" -Severity Information -console $true
Write-Log "Sammle Informationen zum Client" -Severity Information -console $true
$Tasks = 12  # Anzahl der Aufgaben
...
$CLserialNummer = ($computerInfo).BiosSeralNumber
Write-Log "Client Seriennummer: $CLserialNummer" -Severity Information -console $true
$CLMonitorSNObject = get-wmiobject wmimonitorid -namespace root\wmi|foreach-object{($_.SerialnumberID|foreach-object{[char]$_}) -join ""}
$CLMonitorSN = $CLMonitorSNObject -join ", "
Write-Log "Monitor Seriennummern: $CLMonitorSN" -Severity Information -console $true
$i++
Update-Progress $i $Tasks
...
#8. HWNr erfragen
$HWnr = Read-Host -Prompt 'Bitte Hardware ID eingeben: ' 
Write-Log "Hardware Nummer lautet: $HWnr" -Severity Information -console $true
$i++
Update-Progress $i $Tasks
...
if (Test-Path $CSVFile) {
   ...
   #>
    $CustomObject = [PSCustomObject]@{
        ...
        CLmac          = $CLmac
        CLserialNummer = $CLserialNummer
        CLMonitorSN    = $CLMonitorSN
...
#12 System Reset anstoßen
systemreset --factoryreset
Write-Log "System Reset wurde angestoßen und muss manuel fort geführt" -Severity Information -console $true
$i++
Update-Progress $i $Tasks

# Fortschrittsbalken abschließen
Write-Progress -Activity "Fortschritt" -Completed
Write-Log "Ermittlung der Daten abgeschlossen und System Reset angestoßen" -Severity Information -console $true
